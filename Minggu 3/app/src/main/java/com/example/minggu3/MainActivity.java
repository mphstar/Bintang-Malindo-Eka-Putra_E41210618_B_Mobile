...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new MahasiswaAdapter(listMahasiswa, new onItemClickReyccleView() {
            @Override
        ...
        searchv.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextChange(String s) {
                if(s.equals("")){
                    ...
                    ArrayList<MahasiswaModel> searchMahasiswa = new ArrayList<MahasiswaModel>();
                    for (int i = 0; i< listMahasiswa.size(); i++ ){
                        if(listMahasiswa.get(i).getNaam().toLowerCase().contains(s.toLowerCase())){
                            searchMahasiswa.add(listMahasiswa.get(i));
                        }
                    }
                    adapter = new MahasiswaAdapter(searchMahasiswa);
                    rey.setAdapter(adapter);
                }
                return false;
            }
